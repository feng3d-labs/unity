import { AddComponentMenu, RegisterComponent, Renderable, oav, Geometry, Material, Vector3, serialize, serialization, MinMaxCurve, MinMaxCurveMode, MinMaxGradient, MinMaxGradientMode, Color4, RenderAtomic, Scene, Camera, mathUtil, GameObject } from 'feng3d';
...
export class TrailRenderer extends Renderable
{
    ...
    update(_interval?: number)
    {
        if (this.emitting)
        {
            const currentPosition = this.transform.worldPosition.clone();

            // 初始化一个必定添加顶点的值
            let moveDistanceSquared = this.minVertexDistance * this.minVertexDistance * 2;
    ...
    {
        const positions = this.positions.map((v) => v.position);
    ...
    {
        this.positions.unshift({ position, birthTime: Date.now() });
    }
